...
export function FeedPage() {
  ...
  return <main id="main-content" className="feed-page">
    <section className="feed-main" aria-label="Poll feed"><section className="poll-composer" aria-label="Create a poll"><span aria-hidden="true">◯</span><p>What's on your mind today?</p><Link className="create-poll-link" to="/polls/new"><span aria-hidden="true">＋</span> Create poll</Link></section><div className="feed-tabs" aria-label="Feed order"><button type="button" aria-pressed={sort === 'for-you'} onClick={() => setSort('for-you')}>For you</button><button type="button" aria-pressed={sort === 'following'} onClick={() => setSort('following')}>Following</button><button type="button" aria-pressed={sort === 'trending'} onClick={() => setSort('trending')}>Trending</button></div>

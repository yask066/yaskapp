...
function renderPollDetails(poll) {
  ...
  const options = (poll.options || []).map((option) => { const percentage = totalVotes ? Math.round((option.votesCount / totalVotes) * 100) : 0; return `<li><div class="poll-result-label"><span>${escapeHtml(option.text)}</span><strong>${percentage}%</strong></div><div class="poll-result-track"><span class="poll-result-bar" style="width:${percentage}%"></span></div><small>${option.votesCount} votes</small></li>`; }).join('');
  return `<p class="poll-question">${escapeHtml(poll.question)}</p>${poll.description ? `<p class="muted">${escapeHtml(poll.description)}</p>` : ''}<ul class="poll-options">${options || '<li class="muted">No options.</li>'}</ul><div class="poll-footer"><span>${totalVotes} votes</span><span>Poll ID: ${escapeHtml(shortId(poll.id))}</span></div><dl class="poll-metadata"><div><dt>Author</dt><dd>${escapeHtml(poll.authorUsername)}</dd></div><div><dt>Visibility</dt><dd>${escapeHtml(poll.visibility)}</dd></div><div><dt>Status</dt><dd>${escapeHtml(poll.status)}</dd></div><div><dt>Comments</dt><dd>${poll.commentsCount}</dd></div><div><dt>Likes</dt><dd>${poll.likesCount}</dd></div><div><dt>Created</dt><dd>${new Date(poll.createdAt).toLocaleString()}</dd></div></dl>`;
}
